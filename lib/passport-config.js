...
module.exports = function(passport) {
  ...
  // error가 있으면 done에 error를 넘겨준다.

  //궁금한점은 'passport local'을 검색해서 보기
  ...
  passport.use(new FacebookStrategy({
    ...
    callbackURL : 'https://tranquil-scrubland-27033.herokuapp.com/auth/facebook/callback',
    profileFields : ['email', 'name', 'picture']
  }, async (token, refreshToken, profile, done) => {
    console.log('Facebook', profile); // profile 정보로 뭐가 넘어오나 보자.
    try {
      var email = (profile.emails && profile.emails[0]) ? profile.emails[0].value : '';//있으면 받고 없으면 ' '
